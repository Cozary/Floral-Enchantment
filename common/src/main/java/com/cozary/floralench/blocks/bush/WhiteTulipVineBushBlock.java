...
public class WhiteTulipVineBushBlock extends VineBushBlock {
    ...
    public WhiteTulipVineBushBlock() {
        super("white_tulip_bush");
    }

    @Override
    public ItemStack getCloneItemStack(LevelReader worldIn, BlockPos pos, BlockState state) {
        return new ItemStack(ModItems.WHITE_TULIP_BUSH_ITEM.get());
    }
    ...
    public InteractionResult useWithoutItem(BlockState state, Level worldIn, BlockPos pos, Player player, BlockHitResult hit) {
        ...
        if (!flag && player.getMainHandItem().getItem() == Items.BONE_MEAL) {
            ...
            popResource(worldIn, pos, new ItemStack(Items.WHITE_TULIP, j));
            ...
            popResource(worldIn, pos, new ItemStack(ModItems.WHITE_TULIP_VINE_ITEM.get(), j));
            worldIn.playSound(null, pos, SoundEvents.GRASS_FALL, SoundSource.BLOCKS, 1.0F, 0.8F + worldIn.random.nextFloat() * 0.4F);
