...
public class EyeblossomMossyBlock extends MossyBlock {

    public static final MapCodec<EyeblossomMossyBlock> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(Codec.BOOL.fieldOf("open").forGetter((eyeblossomBlock) -> {
            return eyeblossomBlock.type.open;
        })).apply(instance, EyeblossomMossyBlock::new);
    });

    private final EyeblossomMossyBlock.Type type;

    public EyeblossomMossyBlock(String name, EyeblossomMossyBlock.Type type) {
        super(name);
        this.type = type;

    }

    public EyeblossomMossyBlock(Boolean open) {
        super("eyeblossom_mossy_cobblestone");
        this.type = EyeblossomMossyBlock.Type.fromBoolean(open);

    }

    public MapCodec<? extends EyeblossomMossyBlock> codec_() {
        return CODEC;
    }
    ...
    private boolean tryChangingState(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        ...
        if (flag == this.type.open) {
            ...
            EyeblossomMossyBlock.Type eyeblossomblock$type = this.type.transform();
    ...
    public static enum Type {
        ...
        public static EyeblossomMossyBlock.Type fromBoolean(boolean open) {
            return open ? OPEN : CLOSED;
        }

        public Block block() {
            return this.open ? ModBlocks.OPEN_EYEBLOSSOM_MOSSY_COBBLESTONE.get() : ModBlocks.CLOSED_EYEBLOSSOM_MOSSY_COBBLESTONE.get();
        }
        ...
        public EyeblossomMossyBlock.Type transform() {
